...
public class HttpHeaderParser {
	...
	public static BeautyCacheInterface.Entry parseCacheHeaders(NetworkResponse response) {
		...
		BeautyCacheInterface.Entry entry = new BeautyCacheInterface.Entry();
		entry.data = response.data;

		if (BeautyHttpConfig.useServerControl) {
			// 使用服务器的缓存控制
			entry.ttl = softExpire;
		} else {
			// 本地自己的缓存时间的控制
			entry.ttl = now + BeautyHttpConfig.cacheTime * 60000; // 分钟转毫秒
		}
